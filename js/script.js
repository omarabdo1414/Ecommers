...
function updateInternetStatus() {
    noInternet.style.display = window.navigator.onLine ? 'none' : 'block';
}
...
// It creates HTML elements for each product and appends them to the product container
function drawData() {
    ...
    // Fallback for missing color property (fixes typo in one product)
            ...
            <div class="card pt-3">
                ...
                <div class="product-item-action d-flex justify-content-between pr-4 pl-4">
                    <div class="actions-left">
                        <button id="add-btn-${item.id}" class="AddToCartBtn btn btn-primary mb-2" onClick="addTOCartEvent(${item.id})">Add</button>
                        <button id="remove-btn-${item.id}" class="RemoveFromCartBtn btn btn-primary mb-2" onClick="removeFromCart(${item.id})">Remove</button>
                    </div>
...
// Here we toggle the heart icon and update the favorites list in localStorage
